...
const CalculatorPage: React.FC = () => {
  ...
  const renderCalculator = () => {
    ...
    // Convert URL parameter to CalculatorType
    const calculatorType = Object.values(CalculatorType).find(
      type => type.toLowerCase() === (calculator as string).toLowerCase()
    ) || CalculatorType.QuickTips;
    
    switch (calculatorType) {
      case CalculatorType.Loan:
        return <LoanCalculator {...commonProps} />;
      case CalculatorType.Mortgage:
        return <MortgageCalculator {...commonProps} />;
      case CalculatorType.Savings:
        return <SavingsCalculator {...commonProps} />;
      case CalculatorType.FixedDeposit:
        return <FixedDepositCalculator {...commonProps} />;
      case CalculatorType.Currency:
        return <CurrencyConverter rates={rates} ratesDate={ratesDate} isLoading={isLoadingRates} />;
      case CalculatorType.CarLoan:
        return <CarLoanCalculator {...commonProps} />;
      case CalculatorType.SavingsGoal:
        return <SavingsGoalPlanner {...commonProps} />;
      case CalculatorType.BudgetPlanner:
        return <MonthlyBudgetPlanner />;
      case CalculatorType.ExpenseVisualizer:
        return <ExpenseBreakdownVisualizer />;
      case CalculatorType.IncomeVisualizer:
        return <IncomeVisualizer />;
      case CalculatorType.ROI:
        return <RoiCalculator {...commonProps} />;
      case CalculatorType.StockGrowth:
        return <StockGrowthEstimator {...commonProps} />;
      case CalculatorType.Inflation:
        return <InflationCalculator {...commonProps} />;
      case CalculatorType.ProfitMargin:
        return <ProfitMarginCalculator {...commonProps} />;
      case CalculatorType.BreakEven:
        return <BreakEvenCalculator {...commonProps} />;
      case CalculatorType.DebtRepayment:
        return <DebtRepaymentTracker />;
      case CalculatorType.DebtConsolidation:
        return <DebtConsolidationEstimator {...commonProps} />;
      case CalculatorType.Retirement:
        return <RetirementPlanner {...commonProps} />;
      case CalculatorType.EducationFund:
        return <EducationFundCalculator {...commonProps} />;
      case CalculatorType.SalaryTax:
        return <SalaryTaxCalculator {...commonProps} rates={rates} />;
      case CalculatorType.CapitalGainTax:
        return <CapitalGainTaxCalculator {...commonProps} />;
      case CalculatorType.QuickTips:
        return <QuickTips onNavigateToTool={handleSelectCalculator} />;
      case CalculatorType.Glossary:
        return <Glossary scrollToId={glossaryScrollTarget} setScrollToId={setGlossaryScrollTarget} />;
      case CalculatorType.FAQ:
        return <FAQ />;
      case CalculatorType.NetWorth:
        return <NetWorthCalculator {...commonProps} />;
      default:
        return <QuickTips onNavigateToTool={handleSelectCalculator} />;
    }
  };
  ...
  const { calculator } = router.query;
  const calculatorType = calculator ? Object.values(CalculatorType).find(
    type => type.toLowerCase() === (calculator as string).toLowerCase()
  ) : CalculatorType.QuickTips;

  // Calculator categories for sidebar
  ...
  const SidebarNav = ({ categories }: { categories: typeof calculatorCategories }) => (
    <nav className="p-4">
      {categories.length > 0 ? (
        categories.map(category => (
          <div key={category.categoryKey} className="mb-2">
            ...
            {openCategories.includes(category.categoryKey) && (
              <ul className="mt-2 space-y-1 pl-4 border-l-2 border-teal-200 dark:border-teal-80">
                {category.tools.map(tool => (
                  <li key={tool.id}>
                    <button
                      onClick={() => handleSelectCalculator(tool.id)}
                      className={`w-full text-left px-3 py-2 rounded-md transition-colors duration-200 ${
                        calculatorType === tool.id
                          ? 'bg-teal-500 dark:bg-teal-60 text-white font-semibold'
                          : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-70'
        ...
        <div className="px-3 py-2 text-gray-50 dark:text-gray-400 italic">{t.search.noResults}</div>
      )}
    </nav>
  );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-slate-900 text-gray-800 dark:text-gray-200">
      <SEO calculatorType={calculatorType} />
      <header className="bg-white dark:bg-slate-80 shadow-md sticky top-0 z-20 border-b border-transparent dark:border-slate-70">
          ...
          {isSearchVisible ? (
            ...
            <>
              ...
              <div className="flex items-center space-x-2">
                ...
                <button onClick={() => setSearchVisible(true)} className="md:hidden p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-slate-700" aria-label="Open search">
                ...
                <button 
                  ...
                  className="px-4 py-2 text-sm font-semibold text-teal-600 dark:text-teal-300 bg-teal-50 dark:bg-slate-700 rounded-md hover:bg-teal-100 dark:hover:bg-slate-600 transition-colors"
                >
      ...
      <div className="flex flex-grow">
        ...
        {/* Desktop Sidebar */}
        <aside className="hidden md:block w-72 bg-white dark:bg-slate-800 border-r dark:border-slate-700 flex-shrink-0 h-screen sticky top-[72px] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
          <SidebarNav categories={filteredCategories} />
        </aside>
      ...
      <footer className="bg-gray-10 dark:bg-slate-800 text-center p-6 text-gray-60 dark:text-gray-400">
...
// Static generation for static export to work properly
export async function getStaticPaths() {
  // Get all calculator types and convert them to lowercase for URL paths
  const calculatorPaths = Object.values(CalculatorType).map(type => ({
    params: { calculator: type.toLowerCase() }
  }));
  
  return {
    paths: calculatorPaths,
    fallback: false // Don't allow other routes, show 404 for invalid calculator names
  };
}

export async function getStaticProps({ params }: { params: { calculator: string } }) {
  // This function is required for static generation but we don't need to return any specific props
  // The page handles all the logic client-side
  return {
    props: {},
  };
}
