...
const CarLoanCalculator: React.FC<CarLoanCalculatorProps> = ({ onNavigateToGlossary }) => {
  ...
  const pageT = t.carLoanCalculator;
  ...
  const { monthlyPayment, totalInterest, totalPayment, loanAmount, amortizationSchedule } = useMemo(() => {
    ...
    if (P > 0 && annualRate > 0 && years > 0) {
      ...
      if (isFinite(M)) {
        ...
        return {
          ...
          totalPayment: totalPaid + down + tradeIn,
          loanAmount: P,
          amortizationSchedule: schedule,
        };
      }
    }
    return { monthlyPayment: 0, totalInterest: 0, totalPayment: 0, loanAmount: P > 0 ? P : 0, amortizationSchedule: [] };
    ...
    <CalculatorCard title={pageT.title} icon="🚗" description={pageT.description}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{pageT.carPrice}</label>
            <input type="number" value={carPrice} onChange={e => setCarPrice(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm focus:outline-none focus:ring-teal-500 focus:border-teal-500 dark:text-gray-100" />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{pageT.tradeInValue}</label>
            <input type="number" value={tradeInValue} onChange={e => setTradeInValue(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm focus:outline-none focus:ring-teal-500 focus:border-teal-500 dark:text-gray-100" />
          </div>
          <div>
        ...
        <div className="bg-teal-50 dark:bg-slate-700/50 p-6 rounded-lg">
          ...
          <div className="space-y-3">
             <div className="flex justify-between">
                <span className="font-medium text-gray-600 dark:text-gray-300">{pageT.totalLoanAmount}:</span>
      ...
      {renderAmortizationSection()}
       ...
       <div className="mt-8 pt-4 border-t border-gray-200 dark:border-slate-600 text-center">
        <p className="text-sm text-gray-600 dark:text-gray-400">
            {helperT.prompt}{' '}
            <button 
                onClick={() => onNavigateToGlossary('loan-term')}
